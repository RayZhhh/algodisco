...
def main():
    parser = argparse.ArgumentParser(description="Run AlgoBLEU algorithm search.")
    parser.add_argument(
        "--config",
        type=str,
    ...
    method_config = AlgoBLEUSearchConfig(**method_config_data)
    ...
    emb_llm = load_class(
        class_path=config_data.get("emb_llm", {}).get("class_path"),
        kwargs=config_data.get("emb_llm", {}).get("kwargs", {}),
    )
