...
def _rank_based_selection(
    ...
    The selection probability for each item is proportional to rank**(-alpha),
    where rank 1 is best and alpha (exploitation_intensity) controls the
    selection pressure.
...
class AlgoDatabase:
    def __init__(
        ...
        islands: Optional[List["AlgoIsland"]] = None,
    ):
        """Initializes the AlgoDatabase.

        Args:
            config: Method-level configuration for the algorithm database.
            islands: An optional list of pre-initialized AlgoIsland instances.
                     If None, `db_num_islands` new AlgoIsland instances are created
                     based on `config`.
        """
        self.config = config
        ...
        self._weights = {
            "ast": config.db_ast_dfg_weight,
            "embedding": config.db_embedding_weight,
            "behavioral": config.db_behavioral_weight,
        }
        ...
        if islands is not None:
            ...
            self.islands: List[AlgoIsland] = [
                AlgoIsland(
                    ...
                    island_capacity=config.db_max_island_capacity,
                    exploitation_intensity=config.db_selection_exploitation_intensity,
                    weights=self._weights,
                    selection_mechanism=config.db_island_selection_mechanism,
                    survival_mechanism=config.db_island_survival_mechanism,
                    multi_obj_survival_topk=config.db_multi_obj_survival_topk,
                    multi_obj_score_rank_weight=config.db_multi_obj_score_rank_weight,
                )
        ...
        self._process_executor = ProcessPoolExecutor(
            max_workers=self.config.db_num_sim_caculator_workers
        )
        ...
        for island in islands_snapshot:
            ...
            futures.append(
                self._process_executor.submit(
                    ...
                    algo,
                    island.algorithms,
                    self.config.db_algo_sim_calculator,
                    self._weights,
                )
            )
    ...
    def restart_database(self):
        ...
        if self.config.db_max_island_capacity is not None:
            return
        ...
        with self._lock:
            ...
            for island_id in reset_islands_ids:
                ...
                self.islands[island_id] = AlgoIsland(
                    sim_calculator=self.config.db_algo_sim_calculator,
                    island_capacity=self.config.db_max_island_capacity,
                    exploitation_intensity=self.config.db_selection_exploitation_intensity,
                    weights=self._weights,
                    selection_mechanism=self.config.db_island_selection_mechanism,
                    survival_mechanism=self.config.db_island_survival_mechanism,
                    multi_obj_survival_topk=self.config.db_multi_obj_survival_topk,
                    multi_obj_score_rank_weight=self.config.db_multi_obj_score_rank_weight,
                )
...
class AlgoIsland:
    ...
    def single_objective_selection(self, k: int = 1) -> List[AlgoProto]:
        ...
        `exploitation_intensity` parameter from the method config controls
