...
from algodisco.methods.randsample.prompt import PromptAdapter
...
class RandSample(IterativeSearchBase):
    """Core class for the RandSample process, using threading."""
    ...
    def select_and_create_prompt(self) -> Optional[AlgoProto]:
        ...
        with self._lock:
            if not self._template_program:
                return None
            template_program_copy = copy.deepcopy(self._template_program)
    ...
    def _log(self, algo_proto: AlgoProto, is_template: bool = False):
        ...
        # Copy before modifying metadata to avoid affecting the original object
        algo_proto_copy = copy.deepcopy(algo_proto)
        ...
        if self._logger:
            ...
            algo_proto_copy.keep_metadata_keys(self._config.keep_metadata_keys)
            log_entry = algo_proto_copy.to_dict()
            log_entry.update(
                {
    ...
    def register(self, algo_proto: AlgoProto):
        ...
        with self._lock:
            if (
                self._config.max_samples is not None
                and self._samples_count + 1 > self._config.max_samples
            ):
                return
