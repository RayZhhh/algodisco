...
from algodisco.methods.funsearch.database import ProgramsDatabase
from algodisco.methods.funsearch.prompt import PromptAdapter
...
class FunSearch(IterativeSearchBase):
    """Core class for the FunSearch process, using threading."""
    ...
    def __init__(
        ...
        *,
        ...
        self._database = ProgramsDatabase(
            ...
            reset_period=config.db_reset_period,
    ...
    def initialize(self):
        ...
        self._database.register_program(template_proto, island_id=None)
    ...
    def select_and_create_prompt(self) -> Optional[AlgoProto]:
        ...
        # Create a NEW AlgoProto for this candidate
        candidate = AlgoProto(language=self._config.language)
        candidate["parents"] = sorted_programs
    ...
    def extract_algo_from_response(self, candidate: AlgoProto) -> AlgoProto:
        """Parses the 'response_text' inside the candidate and updates 'candidate.program' in-place.

        This method extracts the code block and assigns it to the candidate's program field.
        """
    ...
    def _log(self, algo_proto: AlgoProto, is_template: bool = False):
        ...
        if self._logger:
            ...
            # Add island stats to the log entry
    ...
    def register(self, algo_proto: AlgoProto):
        ...
        with self._lock:
            # Skip register if meeting sample nums.
            if (
                self._config.max_samples is not None
                and self._samples_count >= self._config.max_samples
            ):
