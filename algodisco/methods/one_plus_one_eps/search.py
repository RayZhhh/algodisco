...
from algodisco.methods.one_plus_one_eps.prompt import PromptAdapter
...
class OnePlusOneEPS(IterativeSearchBase):
    """Core class for the (1+1)-EPS process, using threading."""
    ...
    def select_and_create_prompt(self) -> Optional[AlgoProto]:
        ...
        with self._lock:
            if not self._best_program:
                return None
            best_program_copy = copy.deepcopy(self._best_program)
    ...
    def register(self, algo_proto: AlgoProto):
        """Registers a new AlgoProto and updates the best found program if applicable."""
